...
##
		...
		P_X_distance = (self.Pursuer_x- self.Evader_x + 0.01)
		P_Y_distance = (self.Pursuer_y- self.Evader_y + 0.01)
		self.P_Ec_distance_sqr = P_X_distance**2 + P_Y_distance**2

		if abs(P_X_distance) == abs(P_Y_distance) :
			self.Best_Action = 0

		elif abs(P_X_distance) > abs(P_Y_distance) :
			if P_X_distance < 0 :
				self.Best_Action = 4
			else:
				self.Best_Action = 1
